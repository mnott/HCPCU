...
sap.ui.controller("hcpcu.view.app.App", {
	...
	navHandler: function (channelId, eventId, data) {
		...
		if (eventId === "to") {
			this.navTo(data.id, data.data, true, data.type);
	...
	/**
	 * Navigate the Application. If a page is not yet
	 * loaded - e.g. through App.view.js - it will be
	 * loaded lazily.
	 */
  navTo : function (id, data, writeHistory, type) {
    if (id === undefined) {
      /*jQuery.sap.log.error*/console.log("navTo failed due to missing id");
    } else {
      var app = this.getView().app;
      // navigate in the app control
      if (app.getPage(id) == null) {
        /*jQuery.sap.log.info*/console.log("+ Now loading: " + id);
        if(type === 'XML') {
          app.addDetailPage(sap.ui.xmlview(id, "hcpcu.view."+id));
          // this.app.addDetailPage(sap.ui.xmlview("Info", "hcpcu.view.Info"));
        } else {
        	console.log("js");
          app.addDetailPage(sap.ui.jsview(id, "hcpcu.view."+id));
        }
      }
